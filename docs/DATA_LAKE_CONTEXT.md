...
- **`[2026-04-29 → READY 2026-05-05] CoinGecko ingestion universe reduction (top 1,000 by mcap) UNBLOCKED, ready to execute as Phase E.`** Originally paused on the writer-race fix. That dependency cleared 2026-05-05 (see writer-race RESOLVED bullet below + §9 entry 0). Path: re-run `scripts/archive/expand_allowlist.py --n 1000 --min-mcap 1000000 --output data/perp_allowlist.csv` against the now-deduplicated, writer-race-defended pipeline. Pre-flight live-pick audit (16 SHORT legs of Apathy Bleed cohorts C1–C4) already cleared the operational hypothesis — all 16 ranked ≤1,000, median #157. Detail: `reports/apathy_universe_cut_audit_2026_04_29.md` (§1 live-pick audit) + §8 (resolution addendum).
