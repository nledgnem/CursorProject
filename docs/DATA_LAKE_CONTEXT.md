...
**`[CORRECTED 2026-07-24]` Apathy Bleed is EXITED — the "17 still OPEN" snapshot below is stale.** Ground-truth from the Variational trade export (`export-trades.csv` / `export-transfers.csv`): the strategy was fully closed **2026-05-12** (with STABLE/ZEC/DASH stopping out 04-24 / 05-05 / 05-06). The book was never updated, so it — and the 08:00 snapshot + stop runner reading it — reported 17 phantom OPEN positions until reconciled 2026-07-24 (`apathy_bleed_book.csv` now 0 OPEN). **Realized loss: −$5,424.66 venue-exact** (price −$4,619.81 + funding −$804.85, ex-fees; ≈−$6k with inferred taker fees) — this is the figure to anchor on. The book ledger records −$4,669.05 on intended-size notionals; the ~$49 gap is a book-internal artifact, not the loss. See `BACKTEST.md` §22. Historical snapshot below left for provenance:
...
9. **`[2026-09-18]` Asset-identity incident — ticker keys hold other coins' history.** `fact_price` / `fact_marketcap` / `fact_volume` (and silver copies) are keyed by an upper-case ticker; the coin behind it was whatever `data/perp_allowlist.csv` named at fetch time. Live-lake audit against Binance and `/coins/markets` (2026-09-18): **current values are right, history is spliced.** Known windows: **2026-03-04 → 03-30** — rows re-injected from the stale git wide files (`data/curated/*_daily.parquet`, commit 599e5cb) by `incremental_update.py` (`combine_first` let stale values win; fixed on branch `fix/lake-asset-identity`): 72 assets carry another coin's market cap (ETH/SOL/DOGE ≈ $2M, TRX/AVAX/HYPE/ZEC/TON $14–27M, plus stables/wrapped) and ~23 another coin's price (DOGE, SHIB, TRUMP, TON, PUMP, FARTCOIN…); silver passes them unflagged. **2026-01-28** — ~60 tickers re-bound to a different coin (history before is the other coin: NEIRO, BRETT, PNUT, BABY, ARIA, ARC, SHELL, GOAT…). **Pre-2024-05** residue (LINK 2020–2024-01, LUNC/JOE/XAI/TON early 2024). `asset_id = 'BITCOIN'` is a memecoin; real Bitcoin is `'BTC'` (but `fact_markets_snapshot` stamps real Bitcoin `'BITCOIN'`). The MSM basket (and so `funding_regime` / `is_mrf_active`) for decisions 2026-03-09 → 03-30 dropped SOL/DOGE/TRX/AVAX/TON/HYPE/ZEC/PUMP. **Also: CoinGecko's own current history is corrupt for some coins in 2026-01 → 03** (1inch 0.156 vs Binance 0.0925 on 2026-03-22) — a CoinGecko re-fetch is not a safe repair source without validation. Until the repair is applied, exclude these windows or run `python scripts/verify_ingestion_integrity.py --mode asset_identity`. Full detail, repair manifest and apply runbook: `reports/incidents/2026-09-18_asset_identity/README.md`.
10. **`[2026-09-18]` `fact_markets_snapshot` stale 2026-08-04 → 2026-09-18.** CoinGecko began returning `fully_diluted_valuation` ≈ 1e24 for two coins (linqai, peipeicoin-vip); the inferred-Int64 polars frame build raised daily, and Step 0 failures were log-only. Fixed on branch `fix/lake-asset-identity` (explicit schema, Telegram alert on Step 0 failure, content-freshness SLA). The 45 missing days cannot be back-filled (`/coins/markets` is current-only).
...
- **`[2026-09-18]` Asset-identity incident — repair in progress** (branch `fix/lake-asset-identity`; see §9 entries 9–10 and `reports/incidents/2026-09-18_asset_identity/README.md`). Decisions 2026-09-18: repair via dry-run manifest → Render apply (hash-checked, backed up); **allowlist refresh FROZEN** (`data_dictionary.yaml` `ingestion_universe.refresh_frozen`); nightly **non-blocking** integrity alert (`src/exports/lake_integrity.py`), to be promoted to blocking for invalid/duplicate mappings and mass coin-switch events once the repaired lake passes cleanly; identity model = immutable `asset_uid` (today's `asset_id` string) with effective-dated `coingecko_id` / `binance_symbol` in `data/asset_registry.csv`; MSM / funding-regime / MRF outputs recomputed after repair with the as-decided record kept, not overwritten. Track F analysis that joins lake market cap or CoinGecko identity is paused until then.
...
- **`[2026-05-07] universe_eligibility consumer audit (closes [2026-05-06] question).`** Confirmed: no live consumer reads `universe_eligibility.parquet` in the daily Render pipeline. `scripts/fetch_coinglass_data.py:130-157` reads it only as the last fallback in an AUTO source-selection chain (`dim_instrument → fact_funding → perp_listings → universe`); never fires in normal operation since `dim_instrument` provides the primary symbol list. `single_coin_panel.csv::is_perp_active` is computed from Binance `onboardDate` (`scripts/generate_single_coin_panel.py:90-94` and `:409`), zero dependency on `universe_eligibility`. Cohort scan documented in `STRATEGIES.md` §3 reads `is_perp_active` from panel data, no columns from `universe_eligibility`. Risk to May 24 cohort scan: PASS. Caveat: Cohort Shorts project chat is outside this repo; if an undocumented direct read of `universe_eligibility` columns exists in-chat, that path inherits the staleness — verify by asking Dan whether the chat workflow ever references `universe_eligibility.parquet` columns directly. Other readers (validation tooling, golden runs, DuckDB view builder) are manual-only and not on the May 24 critical path.
- **`[2026-05-07 → FIXED 2026-05-07] panel-rebuild brittleness on Binance exchangeInfo failure.`** `scripts/generate_single_coin_panel.py::fetch_binance_exchange_info` was single-attempt with no retry/backoff (compare to other Binance fetches in the same file at lines 155, 239 which already had retry loops). On any failure, `generate_panel_data:358` raised `RuntimeError("Binance exchangeInfo unavailable")` and the entire panel rebuild aborted. `src/exports/panel_generation.py:113-138` caught at the orchestration layer and sent `[PANEL] generation failed (non-fatal): exit=1` to Telegram; `OUTPUT_FINAL` (`/data/single_coin_panel.csv`) kept yesterday's content (atomic rename only on success), and `DEDUP_MARKER` was not advanced so the next run retried naturally. Manifested as today's `[PANEL] generation failed` Telegram alert. Fix: 3-attempt retry with exponential backoff (5s → 15s between attempts), matching the in-file convention. Function signature unchanged; caller's existing `if not ex: raise` path preserved. Companion fallback-to-previous-panel logic intentionally not bundled — separate design decision.
...
### 16.1 The canonical context docs

`[UPDATED 2026-07-24]` START_HERE.md added as the router/authority-map entry point (5th doc).

| File (in repo) | Drive filename | Drive file ID | Role |
|---|---|---|---|
| `START_HERE.md` | `START_HERE.md` | _(pending first export)_ | Router + authority map. No facts; says what to read and, when two docs disagree, which one wins. Read first. |
