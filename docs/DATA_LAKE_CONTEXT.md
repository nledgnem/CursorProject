...
0. **⚠️ CRITICAL 2026-04-29: `fact_price` / `fact_marketcap` / `fact_volume` contain WRONG values (not just stale or missing) for ~139 symbols with bridged variants in the CoinGecko allowlist.** Affected: ETH (~5,000× off), SOL (~700×), DOGE (~120×), SHIB, TRX, AVAX, TON, ZEC, USDT (absent on latest), and ~130 others. The bug is a writer-race in `src/providers/coingecko.py::download_all_coins` (lines 291–308) that keys results by `symbol` while iterating allowlist rows whose `coingecko_id` differs — so duplicate-symbol bridged variants overwrite the canonical slug's data. Until fixed, **prefer `fact_markets_snapshot` for blue-chip mcap/rank** (different code path, NOT affected); **do NOT trust per-symbol joins on `fact_marketcap` / `fact_price` / `fact_volume` for affected tickers.** Single-variant symbols (BTC, BNB, XRP, TAO, DASH, etc.) are correct. Investigation queued as dedicated task; full impact and timeline in `reports/apathy_universe_cut_audit_2026_04_29.md` §4. See also §13 for dependent work pauses.
...
- **`[2026-04-29] CoinGecko ingestion universe reduction PAUSED PENDING DATA-INTEGRITY FIX.`** Reduction work was Step 3-deep when the writer-race bug below was discovered. Original decision (Mads + Dan, 2026-04-29) was to reduce 2,997-row sticky allowlist to top 1,000 by mcap. Pre-flight live-pick audit (16 SHORT legs of Apathy Bleed cohorts C1–C4) cleared the operational hypothesis — all 16 ranked ≤1,000, median #157. But Step 3 surfaced a P0 writer-race that makes the cut inadvisable in isolation. Universe cut becomes the closing step of the dedicated `coingecko-data-integrity-fix-2026-04-29` task. Detail: `reports/apathy_universe_cut_audit_2026_04_29.md`.
- **`[2026-04-29 — P0] download_all_coins writer-race in src/providers/coingecko.py:291-308.`** Fetcher iterates allowlist by row, makes one CoinGecko API call per row using `coingecko_id`, but writes results to a dict **keyed by `symbol`**. For ~139 symbols with duplicate-symbol bridged variants in the allowlist (ETH×4, USDT×19, SOL×5, DOGE×3, etc.), the last iteration wins and earlier results are silently overwritten. Empirical impact on `fact_marketcap` 2026-04-25: ETH at $41M instead of ~$250B (~5,000× error), SOL ~700×, DOGE ~120×, plus ~136 others. `fact_price` / `fact_marketcap` / `fact_volume` all affected. **`fact_markets_snapshot` is NOT affected** (different code path — paginated `/coins/markets`). DO NOT trust `fact_marketcap` / `fact_price` / `fact_volume` for any symbol with bridged variants until fixed. Investigation queued as dedicated task `coingecko-data-integrity-fix-2026-04-29`. See §9 entry 0.
- **`[2026-04-29] dim_asset.coingecko_id is a placeholder.`** All 2,717 rows have `coingecko_id == lowercase(symbol)` (e.g. `BTC→"btc"`, `ETH→"eth"`, `ZEC→"zec"`); zero rows have real CoinGecko slugs (`bitcoin`, `ethereum`, `zcash`). Daily ingestion does NOT use this column (reads allowlist directly), so dormant. But corrupts any analysis joining via `dim_asset.coingecko_id`. Documentation drift: `data_dictionary.yaml` line 115 still describes `fact_price.asset_id` as "Canonical CoinGecko slug … Matches dim_asset.coingecko_id" which is empirically wrong on both halves. Bundle remediation with the writer-race fix in the dedicated task.
