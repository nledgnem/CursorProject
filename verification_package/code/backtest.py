...
class BacktestEngine:
    ...
    def _run_window(
        ...
        for row in dates.iter_rows(named=True):
            ...
            # Dynamic rebalancing: only rebalance if needed
    ...
    def _compute_daily_pnl(
        ...
        if self.funding_enabled and funding is not None:
            ...
            if len(funding_prev) > 0:
                # Simplified: average funding rate
                avg_funding = funding_prev["funding_rate"].mean()
                if self.funding_8h_rate:
                    funding_cost = avg_funding * 3.0  # 3x per day
                else:
