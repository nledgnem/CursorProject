...
class RebalanceSnapshot:
    rebalance_date: date
    divisor: str  # stored as Decimal string
    symbols: List[str]
    weights: List[str]  # Decimal strings
    rebalance_prices: List[str]  # Decimal strings (BTC-denominated)
...
class StateStorage:
    ...
    All numeric values are stored as strings (Decimal-compatible)
    to avoid float ambiguity.
    """
    ...
    def write_index_timeseries(self, df: pd.DataFrame) -> None:
        """
        Persist a reconstructed index DataFrame with columns:
        - date (datetime.date or datetime64)
        - reconstructed_index_value (Decimal string or convertible)
        - daily_divisor (Decimal string or convertible)
        """
        if df.empty:
            return
