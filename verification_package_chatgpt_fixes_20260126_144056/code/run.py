    ...
    try:
        ...
        if is_msm_mode or (experiment_spec and experiment_spec.get("backtest", {}).get("with_regime_eval", False)):
            ...
            if is_msm_mode:
                ...
                # Compute target returns: r_alts_index - r_BTC
                # We'll compute this by building baskets at each date and computing returns
                target_returns_list = []
                
                # Get unique dates from backtest results
                if len(backtest_results) > 0 and "date" in backtest_results.columns:
                    backtest_dates = sorted(backtest_results["date"].unique().to_list())
                    logger.info(f"Computing MSM target returns for {len(backtest_dates)} dates")
                    
                    from datetime import timedelta
                    for i, eval_date in enumerate(backtest_dates):
                        try:
                            # Build MSM basket as-of this date
                            alt_weights = beta_neutral.build_msm_basket(
                                prices, marketcap, volume, eval_date,
                                n=short_leg_config.get("n", 20),
                                min_mcap_usd=config["universe"]["min_mcap_usd"],
                            ...
                            # Get prices for eval_date and next day (for forward return)
                            next_date = eval_date + timedelta(days=1)
                            
                            # Get ALT prices
                            alt_returns_weighted = []
                            for alt_id, weight in alt_weights.items():
                                alt_prices = prices.filter(pl.col("asset_id") == alt_id).sort("date")
                                price_prev = alt_prices.filter(pl.col("date") == pl.date(eval_date.year, eval_date.month, eval_date.day))["close"]
                                price_next = alt_prices.filter(pl.col("date") == pl.date(next_date.year, next_date.month, next_date.day))["close"]
                                
                                if len(price_prev) > 0 and len(price_next) > 0:
                                    ret = (price_next[0] / price_prev[0]) - 1.0
                                    alt_returns_weighted.append(ret * abs(weight))
                            
                            # Get BTC return
                            btc_prices = prices.filter(pl.col("asset_id") == "BTC").sort("date")
                            btc_price_prev = btc_prices.filter(pl.col("date") == pl.date(eval_date.year, eval_date.month, eval_date.day))["close"]
                            btc_price_next = btc_prices.filter(pl.col("date") == pl.date(next_date.year, next_date.month, next_date.day))["close"]
                            
                            if len(alt_returns_weighted) > 0 and len(btc_price_prev) > 0 and len(btc_price_next) > 0:
                                # ALT index return (weighted average)
                                total_weight = sum(abs(w) for w in alt_weights.values())
                                alt_index_return = sum(alt_returns_weighted) / total_weight if total_weight > 0 else 0.0
                    ...
                    logger.warning("No backtest dates available for MSM target computation")
